...
Layout of the manuscript timeline figure:
  ...
  - Middle: HSMM-predicted phase band.
  - Bottom: Expert (ground truth) phase band with %MAE labels at each transition.
  - Title carries Total Error (s) and Mean %MAE.

We use configuration A (Model 1, no OK/NG gate), the headline configuration. The
best-performing case under A is internal Case 4, labeled "Case XII" under the
manuscript Roman-numeral scheme.

Output: <AMVIDEO_FIGURES, default ./figures>/figure7_caseXII.png
"""
...
from temporal_analysis import (
    ...
    learn_transition, estimate_transitions, compute_timepoint_errors,
    build_prediction_timeline, get_video_durations, set_duration_source,
)

# ── best-performing case under the headline configuration A ─────────────
BEST_CASE = 4             # internal Case 4 → "Case XII" in the manuscript scheme
BEST_FOLD = 1             # fold whose held-out set contains case 4

# Internal case # → manuscript Roman label (all 20 cases; see config.FOLD_TEST_CASES).
CASE_ROMAN = {
    2: "I", 3: "II", 8: "III", 9: "IV", 10: "V", 11: "VI", 12: "VII", 15: "VIII",
    16: "IX", 17: "X", 1: "XI", 4: "XII", 5: "XIII", 6: "XIV", 7: "XV", 13: "XVI",
    14: "XVII", 18: "XVIII", 19: "XIX", 20: "XX",
}
...
PHASE_TO_TRANSITION_NAME = list(config.TRANSITION_PHASES)


def compute_case_data(case_num: int, fold: int, vis_smooth_window: int = 300):
    """Reproduce the configuration-A pipeline for one case and return plot data.

    `vis_smooth_window` (seconds) is an *additional* smoothing window applied only
    to the probability curves rendered in the top panel. The HSMM runs on the
    standard config.SMOOTHING_WINDOW-smoothed sequence, so onset positions remain
    identical to the numerical results.
    """
    ...
    times, probs = build_prediction_timeline(str(pred_csv), case_num, offsets)
    smoothed = smooth_probabilities(times, probs)                                # for HSMM
    smoothed_vis = smooth_probabilities(times, probs, window=vis_smooth_window)  # for plot
    predicted = estimate_transitions(times, smoothed, logA)      # {phase: time or None}
    ...
    # Ordered, monotonic onset list for the predicted phase band.
    boundaries = []
    prev = float(times[0])
    for name in PHASE_TO_TRANSITION_NAME:
        t = predicted.get(name)
        prev = max(prev, t) if t is not None else prev
        boundaries.append(prev)
    return {
        "times": times, "smoothed": smoothed, "smoothed_vis": smoothed_vis,
        "boundaries": boundaries, "predicted": predicted,
        "expert": expert, "errors": errors, "total_dur": total_dur,
    }
...
def plot_figure(data: dict, case_label: str, out_path: Path) -> None:
    ...
    mean_pct = float(np.mean(pct_errors)) if pct_errors else 0.0

    # Expert boundaries: first onset per phase name.
    expert_by_name = {}
    for tsec, name in sorted(expert_transitions, key=lambda x: x[0]):
        expert_by_name.setdefault(name, tsec)

    # ── Figure layout ───────────────────────────────────────────────────
    fig = plt.figure(figsize=(14.0, 6.0))
    # Use a 3-row gridspec: probabilities (top), HSMM band, Expert band + labels (bottom)
    ...
    ax_dp.text(-0.012, 0.5, "HSMM Prediction:", transform=ax_dp.transAxes,
    ...
    # Dashed connectors between HSMM and expert boundaries; %MAE label at expert
    for i, name in enumerate(PHASE_TO_TRANSITION_NAME):
        if i >= len(boundaries):
            break
        if name not in expert_by_name or predicted.get(name) is None:
            continue
        dp_t = predicted[name]
    ...
    # Iterate the errors dict directly so labels stay aligned even if a phase is skipped.
    print(f"  per-boundary %MAE: " +
          ", ".join(f"{name}={v['absolute_error'] / total_dur * 100:.2f}%"
                    for name, v in errors.items()))


def main() -> None:
    data = compute_case_data(BEST_CASE, BEST_FOLD, vis_smooth_window=600)
